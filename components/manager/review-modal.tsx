...
// Review Modal (matches exact ROPA layout with inline rejection)
export function ManagerReviewModal({ onClose }: { onClose: () => void }) {
  const [step, setStep] = useState(1);
  const [isRejecting, setIsRejecting] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (isRejecting && scrollRef.current) {
      setTimeout(() => {
  ...
  const DisplayBlock = ({ className = "h-11" }: { className?: string }) => (
    <div className={`w-full bg-white/10 rounded-xl px-4 py-3 border border-white/5 ${className}`} />
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm">
      <div className="bg-[#e4e5ea] border border-gray-300 rounded-[2rem] w-full max-w-4xl max-h-[95vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Note: I am forcing light theme here specifically for the modal view as wireframes look very light grey */}
        ...
        <div ref={scrollRef} className="p-6 sm:p-8 overflow-y-auto space-y-8 relative text-gray-800 pointer-events-auto">
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              {/* Inline Reject Block Triggered by Reject Button */}
              {isRejecting && (
                <div className="pt-6 border-t border-gray-300 animate-in fade-in slide-in-from-bottom-4 duration-300">
                   <h4 className="font-bold text-sm mb-3">เหตุผลในการปฏิเสธ <span className="text-red-500">*</span></h4>
                   <textarea 
                     rows={5}
                     className="w-full bg-[#c3c1c2] border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500 resize-none text-gray-900 shadow-inner"
                   />
                </div>
              )}
            </div>
        ...
        <div className="p-6 md:px-8 border-t border-gray-300 flex justify-between items-center rounded-b-[2rem]">
          {step === 1 ? (
            ...
            <div className={`w-full flex ${isRejecting ? 'justify-between gap-4' : 'justify-between items-center'}`}>
              {!isRejecting ? (
                <>
                  ...
                  <div className="flex gap-4">
                    <button onClick={onClose} className="px-8 py-3 rounded-lg text-white transition-colors font-bold text-[15px] bg-[#898989] hover:bg-[#6c6c6c] shadow-md w-32">
                      Completed
                    </button>
                    <button onClick={() => setIsRejecting(true)} className="px-8 py-3 rounded-lg text-white transition-colors font-bold text-[15px] bg-[#898989] hover:bg-red-600 shadow-md w-32">
                      Reject
                    </button>
                  </div>
                </>
                ...
                <>
                  <button onClick={() => setIsRejecting(false)} className="flex-1 py-3 rounded-sm bg-white text-gray-900 border border-gray-300 hover:bg-gray-100 transition-colors font-bold text-[15px] shadow-sm">
                    ยกเลิก
                  </button>
                  <button onClick={onClose} className="flex-1 py-3 rounded-sm bg-white text-gray-900 border border-gray-300 hover:bg-gray-100 transition-colors font-bold text-[15px] shadow-sm">
                    ยืนยัน
                  </button>
                </>
              )}
