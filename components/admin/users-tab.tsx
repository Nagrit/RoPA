...
export function UsersTab() {
  ...
  const handleEditClick = (user: any) => {
    ...
    setFormData({
      ...
      email: user.email,
      password: '', // Usually keep password blank for security during edits
      role: user.role,
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full pt-4">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
            onClick={() => {resetForm(); setEditingUserId(null); setIsModalOpen(true); }}
                ...
                <td className="px-8 py-6 text-right">
                  <div className="flex items-center justify-end gap-3 text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-2 hover:text-white transition-colors"><MoreVertical size={20} /></button>
                    <button
                      onClick={() => handleDelete(u.id)}
                      className="p-2 hover:text-red-400 transition-colors"><Trash2 size={20} />
                    </button>
                  </div>
                </td>
              </tr>
          ...
          <div className="relative bg-[#0f172a] border border-slate-800 w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 border-b border-slate-800 bg-slate-900/20">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-bold text-white tracking-tight"> {editingUserId ? 'Edit Account' : 'New Account'} </h3>
