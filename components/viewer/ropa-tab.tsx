...
import { Search, Filter, Calendar, Users, ChevronDown, CheckCircle2 } from 'lucide-react';
import { ViewerReviewModal } from './review-modal';

const RECORDS = [
  { id: 'ระบบจัดการข้อมูลพนักงาน xxx-01', dept: 'xxxx', type: 'xxx', status: 'Completed', date: '2026-05-05' },
  { id: 'ระบบxxxx xxx-02', dept: 'xxxx', type: 'xxx', status: 'Completed', date: '2026-05-05' },
  { id: 'ระบบxxxx xxx-03', dept: 'xxxx', type: 'xxx', status: 'Completed', date: '2026-05-05' },
  { id: 'ระบบxxxx xxx-04', dept: 'xxxx', type: 'xxx', status: 'Completed', date: '2026-05-05' },
];

export function ViewerRopaTab() {
  const [isReviewOpen, setIsReviewOpen] = useState(false);

  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 h-full flex flex-col max-w-6xl mx-auto relative">
      ...
      {/* Header Panel */}
      <div className="bg-[#0b1429]/40 backdrop-blur-xl border border-slate-800/60 rounded-[32px] p-8 shadow-2xl flex justify-between items-start relative z-10">
        <div>
      ...
      <div className="flex flex-wrap gap-4 relative z-10">
        ...
        <button className="flex items-center justify-between min-w-[180px] bg-slate-800/50 hover:bg-slate-700 border border-slate-700/50 px-5 py-3.5 rounded-xl transition-all text-xs font-bold text-slate-300 uppercase tracking-widest">
          <div className="flex items-center gap-2"><Filter size={16} className="text-emerald-400" /> All</div>
        </button>
      </div>

      {/* Main Table Panel */}
      <div className="bg-[#0b1429]/30 backdrop-blur-xl border border-slate-800/60 rounded-[32px] flex flex-col shadow-2xl flex-1 overflow-hidden relative z-10">
        {/* Toolbar */}
          ...
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-900/40 border-b border-slate-800/60">
              <tr>
                ...
                <th className="py-5 px-8 text-[11px] font-black uppercase text-slate-600 tracking-[0.15em]">Department</th>
                ...
                <th className="py-5 px-8 text-[11px] font-black uppercase text-slate-600 tracking-[0.15em] text-center">จัดการ</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/40">
              {RECORDS.map((rec, i) => (
                <tr key={i} className="hover:bg-slate-800/20 transition-colors group">
                  ...
                  <td className="px-8 py-5 font-bold text-slate-400">{rec.dept}</td>
                  <td className="px-8 py-5 font-bold text-slate-400">{rec.type}</td>
                  <td className="px-8 py-5">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-[10px] font-black uppercase tracking-wider">
                  ...
                  <td className="px-8 py-5 text-center">
                    <button 
                      ...
                      className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-[11px] uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-blue-900/20 active:scale-95 opacity-0 group-hover:opacity-100"
                    >
                      View
                    </button>
                  </td>
      ...
      {isReviewOpen && <ViewerReviewModal onClose={() => setIsReviewOpen(false)} />}
    </div>
  );
}
